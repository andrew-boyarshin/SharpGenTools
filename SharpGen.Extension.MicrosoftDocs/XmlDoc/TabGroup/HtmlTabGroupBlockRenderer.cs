...
{
    public class HtmlTabGroupBlockRenderer : XmlDocObjectRenderer<TabGroupBlock>
    {
        protected override void Write(XmlDocRenderer renderer, TabGroupBlock block)
        {
        ...
        private void WriteTabHeaders(XmlDocRenderer renderer, TabGroupBlock block, string groupId)
        {
        ...
        private void WriteTabSections(XmlDocRenderer renderer, TabGroupBlock block, string groupId)
        {
            for (var i = 0; i < block.Items.Length; i++)
            {
                var item = block.Items[i];
        ...
        private void AppendGroupId(XmlDocRenderer renderer, string groupId, TabItemBlock item)
        {
            renderer.Write(groupId);
            renderer.Write("_");
            renderer.Write(item.Id);
